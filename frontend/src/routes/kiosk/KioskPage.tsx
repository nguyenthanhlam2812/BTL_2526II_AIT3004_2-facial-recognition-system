...
export default function KioskPage() {
  ...
  const scanState = useMemo(() => {
    ...
    if (result?.matched && result.attendance_status !== "inactive_employee") return "success";
    if (result && !result.matched) return "fail";
    if (result?.attendance_status === "inactive_employee") return "fail";
    return "idle";
  }, [isPending, result]);
...
function ResultOverlay({ result }: { result: AttendanceFrameResponse }) {
  const status =
    result.attendance_status === "multiple_faces"
      ...
      : result.attendance_status === "inactive_employee"
        ? { color: "yellow", label: "Ngưng hoạt động" }
        : result.matched
