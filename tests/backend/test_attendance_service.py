...
def test_recognize_attendance_frame_rejects_inactive_employee(
    db_session,
    monkeypatch,
):
    employee = Employee(
        employee_code="E001-INACTIVE",
        full_name="Nguyen Van Inactive",
        department="IT",
        position="Engineer",
        status="inactive",
    )
    ...
    assert response.matched is True
    assert response.attendance_status == "inactive_employee"
    assert response.employee is not None
    assert response.employee.id == employee.id
    assert response.event_id is None
    assert db_session.scalar(select(func.count()).select_from(AttendanceEvent)) == 0
...
def test_recognize_attendance_frame_inactive_does_not_record_event(
    db_session,
    monkeypatch,
):
    """Inactive employees are recognized but no attendance event is persisted."""
    employee = Employee(
        employee_code="E002-INACTIVE",
        full_name="Tran Thi Inactive",
        department="HR",
        position="Manager",
        status="inactive",
    )
    ...
    # Call both check_in and check_out — neither should create events
